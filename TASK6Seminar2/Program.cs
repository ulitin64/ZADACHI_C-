...
int GetRandomNumberInRange(int minNumber, int maxNumber)
{
    ...
    int GetMaxDigitFomNumber(int number)
    {
    ...
    for (int i = 0; i<10; i++)
{
    ...
    int maxDigit = GetMaxDigitFomNumber(randomNumber);
    Console.WriteLine($"наибольшая цифра для числа {randomNumber} является {maxDigit}");
}